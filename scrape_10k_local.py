#!/usr/bin/env python3
"""
S&P 500 10-K Filings Scraper - Local Version
Scrapes 10-K filings from SEC EDGAR and saves to local CSV
...
INPUT_CSV_PATH = None
OUTPUT_DIR = "/home/user/Case-2/10k_filings_output"
...
class SECFilingScraper:
    ...
    def scrape_all(self, csv_path: str, years: int = 5, max_companies: int = None) -> pd.DataFrame:
...
def main(input_csv: str, max_companies: int = None):
    """Main execution function"""
    print("="*80)
    print("S&P 500 10-K Filings Scraper - Local Version")
    print("="*80)

    if not os.path.exists(input_csv):
        print(f"\n⚠️  ERROR: Input CSV not found: {input_csv}")
        return None

    print(f"\n📂 Reading companies from: {input_csv}")
    print(f"📁 Saving CSV to: {OUTPUT_DIR}")
    print(f"📅 Downloading filings from last {YEARS_TO_DOWNLOAD} years")
    print(f"👤 User-Agent: {USER_AGENT}")
    print(f"📝 Output: Parsed text (HTML tags removed)\n")

    # Create scraper and run
    scraper = SECFilingScraper(output_dir=OUTPUT_DIR)
    df = scraper.scrape_all(csv_path=input_csv, years=YEARS_TO_DOWNLOAD, max_companies=max_companies)

    if df is not None and len(df) > 0:
        print("\n✅ Done! CSV file saved locally.")
        print(f"📊 File: {OUTPUT_DIR}/sp500_10k_filings.csv")
